<script>
	$(document).ready(function() {
		...
		$(document).on('click','.read-more',function(e){
			...
			let data = curCount;
			$(".content").append($('<div>').load("/admin/viewCatalogs/",{action:"loadContent", id:data, idCatalog:"<?php echo $catalog[0]['id']; ?>"}, function() {
...
<div class="container">
    <div class="panel settings-panel">
		<div class="title-panel">
			<span class="c-white fs-medium ff-SansNarrow"><?php echo $catalog[0]['title']; ?></span>
			<?php if (!empty($total)): ?>
				<span class="c-white fs-medium ff-SansNarrow" id="info-count-total"></span>
			<?php endif; ?> 
			<a href="/admin/addProduct/<?php echo $catalog[0]['id']; ?>">
				<div class="b-plus-title "></div>
			</a>
		</div>
		<?php if (empty($list)): ?>
			<div class="container-empty-list">
				<span class="fs-medium ff-SansNarrow">Список изделий пуст</span>
			</div>
        <?php else: ?>
				...
				<?php foreach ($list as $val): ?>
					<div class="d-tr" id="delete-block" data-id="<?php echo $val['id']; ?>">
						<span class="d-td" style="flex-grow: 3;">
							<a class="d-td-action" href="/product/<?php echo $val['id']; ?>">
								<?php echo $val['title']; ?>
							</a>
						</span>
						<span class="d-td">
							<a class="d-td-action" href="/admin/editProduct/<?php echo $val['id']; ?>">Редактировать</a>
						</span>
						<span class="d-td">
